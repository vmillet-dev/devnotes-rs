...
export async function startApplication(): Promise<void> {
  ...
  // ⚠️ Before `locale.restore()`, which reads the language out of it.
  settings.restore();
  locale.restore();
  // ⚠️ After `restore()`: the front creates the tray by giving it its labels, and earlier
  // would push the default language and a setting the user had changed.
  tray.start();

  // ⚠️ Before the vault: it says which library is open, and opens that library's own
  // preference file — reading the wrong one seeds a library that is full.
  await libraries.load();

  // ⚠️ Before the first render: the shell renders nothing until this answers, rather than
