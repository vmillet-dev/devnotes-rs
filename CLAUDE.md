...
- **The 12-character floor is for choosing a phrase, never for typing one** (`vault::MINIMUM_LENGTH`, crossing as `MINIMUM_PASSPHRASE_LENGTH`). ⚠️ A library created under the old floor of eight still opens — the gate and the import prompt check nothing but emptiness — and `VaultStore.unlock` says so after every unlock. → "The floor".
