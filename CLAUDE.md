...
- **The 12-character floor is for choosing a phrase, never for typing one** (`vault::MINIMUM_LENGTH`, crossing as `MINIMUM_PASSPHRASE_LENGTH`). ⚠️ A library created under the old floor of eight still opens — the gate and the import prompt check nothing but emptiness — and `unlock_vault` answers `belowMinimum` so every unlock says so. → "The floor".
...
- **The launch copy is taken before the sweeps** (`backup::rotate`, `VACUUM INTO` — under WAL the file alone is no snapshot), with its key file, at most one a day. **Restoring** empties the `Mutex` before a file moves, sets the live pair aside in `replaced/` and matches the id against the listing. → "The copies, and putting one back".
