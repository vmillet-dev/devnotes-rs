...
- **The launch copy is taken before the sweeps** (`backup::rotate`, `VACUUM INTO` — under WAL the file alone is no snapshot), with its key file and `attachments/` (unless `backupAttachments` says `"false"`), at most one a day. **Restoring** empties the `Mutex` before a file moves, sets the live pair aside in `replaced/` (and `attachments/` when the copy carries its own) and matches the id against the listing. → "The copies, and putting one back".
