...
/// Trimmed and never blank: [`is_taken`] folds case but not spaces, so "Perf" and " Perf "
/// would otherwise coexist, identical on screen.
...
/// Whether `name` is already held, case aside, by anything but `except_id`, so a rename can
/// correct its own case. Decided here, not by a unique index: the names are sealed.
