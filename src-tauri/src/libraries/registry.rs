//! The registry of libraries on disk, beside them: which exist, where, and which is open.
//! Every rule takes the profile's path rather than an `AppHandle`, which a unit test cannot build.
...
pub(crate) fn registry_in(profile: &Path) -> Registry {
...
pub(crate) fn create_in(profile: &Path, name: &str) -> Result<LibraryEntry, StorageError> {
...
pub(crate) fn point_at(profile: &Path, id: &str) -> Result<(), StorageError> {
    let mut registry = registry_in(profile);
...
pub(crate) fn rename_in(profile: &Path, id: &str, name: &str) -> Result<(), StorageError> {
    let mut registry = registry_in(profile);
...
pub(crate) fn delete_in(profile: &Path, id: &str) -> Result<(), StorageError> {
    let mut registry = registry_in(profile);
