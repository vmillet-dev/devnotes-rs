...
safe because an attachment is **never rewritten**: a new one is a new id, an import remaps the
id, and a purge or the orphan sweep only unlinks — so a file deleted from the library stays
whole in the copies. `backups/` sits inside the library's directory, on the same volume; a
file system that refuses a link (FAT32, exFAT, some network shares) gets a copy instead.
⚠️ A name already in the target is **left alone**, never copied over: it is the same
attachment and often the very same file, and `std::fs::copy` onto a link to its own source
truncates the bytes it is about to read. A restore that could not move the live
`attachments/` aside links into it, which is when that happens. What a link does not do is
protect against the file being damaged on disk; a copy on the same disk barely did either.
...
disk — and that the attached files are linked rather than duplicated.

`list_backups` answers `Backup { id, takenAt, bytes, openable }`, newest first. `bytes` is
the database alone: the attachments are shared with the library and the other copies, and
counting them in each would make three copies look like three times the disk. ⚠️
...
- ⚠️ `vault.json` travels, unlike `recovery::set_aside` where it deliberately stays. The
  ...
  opens nothing.
- ⚠️ `attachments/` follows the copy. The live directory goes into
  `replaced/<timestamp>/` with the database — an attachment added since is kept there, not
  lost — and the copy's is linked back.
